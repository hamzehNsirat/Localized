...
import quotationsApi from "../../../api/retailerAPIs/quotations";
  ...
  const purchaseQuotation = async () => {
    try {
      ...
      const payload = {
        ...
        buyerId: parseInt(quotation.requesterId),
        supplierId: parseInt(quotation.supplierId),
        paymentAmount: quotation.total,
        paymentMethod: paymentMethod,
      };
      console.log(payload);
      const response = await quotationsApi.createPurchase(payload);
      if (response?.body?.success) {
        console.log("Purchase done");
        navigate(`/retailer/manageQuotations/${quotation.id}/finishPurchase`, {
          state: { quotation, paymentMethod },
        });
      } else if (!response.body.details.success) {
        console.error("Failed to purchase quotation", err);
      }
    } catch (err) {
      console.error("Failed to purchase quotation", err);
    } finally {
